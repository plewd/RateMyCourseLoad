...
```bash
ratemycourseload$ sam local invoke NetCodeWebAPIServerless --event events/event.json
```

The AWS SAM CLI can also emulate your application's API. Use the `sam local start-api` command to run the API locally on port 3000.

```bash
ratemycourseload$ sam local start-api
ratemycourseload$ curl http://localhost:3000/
```
...
```bash
ratemycourseload$ sam logs -n NetCodeWebAPIServerless --stack-name ratemycourseload --tail
```
...
```bash
ratemycourseload$ dotnet test test/ServerlessAPI.Tests/ServerlessAPI.Tests.csproj 
```
...
```bash
sam delete --stack-name ratemycourseload
```
